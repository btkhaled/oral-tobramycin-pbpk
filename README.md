...
Symlinks for legacy compatibility: `pksim → 03_PKSIM`, `results → 04_RESULTS`, `data → 01_COMPOUND_DATA`, `manuscript → 06_MANUSCRIPT`, `slides → 07_SLIDES`, `docs → 03_PKSIM/docs`. Numbering: `00-01` foundations, `02` exploratory, `03` PK-Sim, `04-05` results→analysis, `06-07` valorisation.
...
- **Snapshots** `01_COMPOUND_DATA/snapshots/` (5 JSON: Amikacin 78K → Tobramycin 64K → Oral 22K → QD/BID 28K) via `data/snapshots/` symlink — generated by `03_PKSIM/model/make_tobramycin_json.py` (portable).
...
> **Reproducibility:** `Rscript run_all.R` (14 steps, via symlinks `pksim→03_PKSIM`, `results→04_RESULTS`, `data→01_COMPOUND_DATA`) reproduces everything — `04_RESULTS/MANIFEST.csv` + `05_ANALYSIS/MANIFEST.csv` prove it. Validated `00`/`01`/`02`/`03`/`04`/`05` → `06` 185 pp.
